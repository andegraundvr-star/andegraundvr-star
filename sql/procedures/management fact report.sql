...
GO
/****** Object:  StoredProcedure [dbo].[ЗначениеБонусаДляПодразделений]    Script Date: 01.12.2025 14:18:47 ******/
SET ANSI_NULLS ON
GO
...
if object_id (N'tempdb..#ДобавлениеСредневзвешенногоБонуса') is not null drop table #ДобавлениеСредневзвешенногоБонуса;
-- Сначала агрегируем по сотрудникам
WITH АгрегацияПоСотрудникам AS (
    SELECT 
        Дата,
        Сотрудник,
        SUM(Вес) as Общий_вес,
        SUM(
            CASE 
...
select 
 ...
 д.Дата,
    ...
    д.Вес,    
    д.ПлановоеЗначение,
    д.ФактическоеЗначение,
    а.Общий_вес,
    CASE 
        WHEN а.Общий_вес = 0 THEN 0
        ELSE а.Сумма_взвешенных / а.Общий_вес
    END as Средневзвешенный_процент_выполнения


into #ДобавлениеСредневзвешенногоБонуса 
from #ЗначениеБонусаДляПодразделений д

JOIN АгрегацияПоСотрудникам а 
    ON д.Сотрудник = а.Сотрудник AND д.Дата = а.Дата;
